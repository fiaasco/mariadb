...
testinfra_hosts = testinfra.utils.ansible_runner.AnsibleRunner(
    os.environ['MOLECULE_INVENTORY_FILE']).get_hosts('mariadb-ubuntu-bionic')


def test_automysqlbackup_package(host):
    """ check if packages are installed
    """
    assert host.package('automysqlbackup').is_installed
